...
class LiquidCircleEffect : LiquidLoadEffect {
    ...
    override func setupShape() -> [LiquittableCircle] {
        return Array(0..<NumberOfCircles).map { i in
            ...
            let center = CGMath.circlePoint(center: frame.center.minus(frame.origin), radius: self.radius - self.circleRadius, rad: angle)
    ...
    override func resize() {
        let moveVec = moveCircle!.center.minus(point: loader.center.minus(loader.frame.origin)).normalized()
